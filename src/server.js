...
// SETTINGS
botRoute('get', '/settings', (req,res) => {
  const d = readJson(getBotPaths(req.user.id).flows, {});
  res.json({
    botName:              d.botName || '',
    sysPrompt:            d.sysPrompt || '',
    fallbackAI:           d.fallbackAI || false,
    workingHours:         d.workingHours || {},
    aiResponseDelay:      d.aiResponseDelay || 0,
    aiPauseWords:         d.aiPauseWords || [],
    messageDebounceSeconds: d.messageDebounceSeconds || 4,
    // Return masked keys so frontend knows if they're set
    anthropicKeySet:      !!(d.anthropicApiKey || process.env.ANTHROPIC_API_KEY),
    groqKeySet:           !!(d.groqApiKey      || process.env.GROQ_API_KEY),
  });
});
botRoute('post', '/settings', async (req,res) => {
  const p = getBotPaths(req.user.id);
  const d = readJson(p.flows, { flows: [] });

  // Save all settings
  Object.assign(d, req.body);

  // Save API keys if provided (not empty strings)
  if (req.body.anthropicApiKey && req.body.anthropicApiKey.trim()) {
    d.anthropicApiKey = req.body.anthropicApiKey.trim();
  }
  if (req.body.groqApiKey && req.body.groqApiKey.trim()) {
    d.groqApiKey = req.body.groqApiKey.trim();
  }

  writeJson(p.flows, d);

  // Reinitialize bot AI clients with new keys
  const bot = botManager.getInstanceIfExists(req.user.id);
  if (bot) {
    const anthropicKey = d.anthropicApiKey || process.env.ANTHROPIC_API_KEY;
    const groqKey      = d.groqApiKey      || process.env.GROQ_API_KEY;

    if (anthropicKey) {
      try {
        const Anthropic = require('@anthropic-ai/sdk');
        bot.ai = new Anthropic({ apiKey: anthropicKey });
      } catch {}
    }
    if (groqKey) {
      try {
        const Groq = require('groq-sdk');
        bot.groq = new Groq({ apiKey: groqKey });
        bot.groqAvailable = true;
        bot.groqRetryAt   = null;
      } catch {}
    }
    // Apply other settings immediately
    if (d.botName) bot.botName = d.botName;
  }
...
botRoute('get', '/campaigns/segments', (req,res) => {
  ...
  const p   = getBotPaths(uid);
  const bot = botManager.getInstanceIfExists(uid);

  // Merge live sessions + persisted sessions.json + all known conversations
  // This ensures no lead is ever lost even after bot restart
  const liveSessions  = bot ? bot.getSessions() : {};
  const savedSessions = readJson(p.base + '/sessions.json', {});
  const convs         = readJson(p.conv, {});

  // Build merged session map: live wins over saved
  ...
  const segments = { potencial_compra:[], novos:[], todos_ativos:[], finalizados:[] };

  for (const jid of allJids) {
    const sess = liveSessions[jid] || savedSessions[jid] || {};
    const conv = convs[jid] || {};
    const msgs = conv.messages || [];

    // Skip leads that never started AND have no conversations
    if (!sess.started && !msgs.length) continue;

    const la  = sess.lastActivity || (msgs.length ? new Date(msgs[msgs.length-1].time||0).getTime() : 0);
    const im  = now - la;
    const lm  = msgs.slice(-1)[0];

    const lead = {
      jid,
      contact:         conv.contact || jid.replace('@s.whatsapp.net',''),
      lastMsg:         lm?.text?.substring(0,60) || '',
      lastActivity:    la,
      flowIndex:       sess.currentFlowIndex ?? -1,
      remarketingCount: sess.remarketingCount || 0,
      finished:        sess.finished || false,
      pausedByHuman:   sess.pausedByHuman || false,
      msgCount:        msgs.length,
    };

    if (lead.finished) { segments.finalizados.push(lead); continue; }
    if (lead.pausedByHuman) { segments.todos_ativos.push(lead); continue; }
  ...
  Object.values(segments).forEach(arr => arr.sort((a,b) => (b.lastActivity||0) - (a.lastActivity||0)));
...
botRoute('post',  '/campaigns/:id/send', async(req,res) => {
  ...
  const sessions=bot?bot.getSessions():{};
  const savedSessions = readJson(p.base + '/sessions.json', {});
  const allSess = { ...savedSessions, ...sessions }; // live wins over saved
  const convs=readJson(p.conv,{});
  const now=Date.now(); const H6=21600000,H24=86400000;
  const alreadySent=new Set(camp.sentJids||[]);
  let jids=[];
  if(camp.customJids?.length){jids=camp.customJids.filter(j=>!alreadySent.has(j));}
  else{
    const allJids = new Set([...Object.keys(allSess), ...Object.keys(convs)]);
    for(const jid of allJids){
      const sess=allSess[jid]||{}; const conv=convs[jid]||{};
      if(!sess.started&&!(conv.messages?.length))continue;
      if(alreadySent.has(jid))continue;
      if(sess.pausedByHuman)continue;
      const im=now-(sess.lastActivity||0); const seg=camp.segment;
      if(seg==='todos_ativos'&&!sess.finished)jids.push(jid);
      else if(seg==='potencial_compra'&&!sess.finished&&sess.currentFlowIndex>0)jids.push(jid);
      else if(seg==='novos'&&!sess.finished&&im<=H24)jids.push(jid);
      else if(seg==='finalizados'&&sess.finished)jids.push(jid);
    }
  }
...
botRoute('post','/ai/chat', async(req,res)=>{
  ...
  const userFlows = readJson(getBotPaths(req.user.id).flows, {});
  const apiKey = userFlows.anthropicApiKey || process.env.ANTHROPIC_API_KEY;
  if(!apiKey||apiKey==='sk-ant-SUA_CHAVE_AQUI')return res.json({reply:'Configure a Anthropic API Key nas Configurações do bot'});
...
botRoute('get','/groq/status',(req,res)=>{
  ...
  const uf = readJson(getBotPaths(req.user.id).flows, {});
  const hasGroq = !!(uf.groqApiKey || process.env.GROQ_API_KEY);
  res.json({configured:hasGroq,available:b?b.groqAvailable:hasGroq,retryAt:b?b.groqRetryAt:null});
});
