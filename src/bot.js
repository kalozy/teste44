...
// Paths resolved dynamically per-instance via this.dataDir
...
function loadFlows(flowsPath) {
  const FLOWS_PATH = flowsPath || STANDALONE_DATA + '/flows.json';
  const now = Date.now();
...
function loadStats(statsPath) {
  const STATS_PATH = statsPath || STANDALONE_DATA + '/stats.json';
  try { return JSON.parse(fs.readFileSync(STATS_PATH, 'utf8')); }
  catch { return { totalMessages:0, totalContacts:0, flowHits:{}, dailyMessages:{} }; }
}
function saveStats(s, statsPath) { const STATS_PATH = statsPath || STANDALONE_DATA + '/stats.json'; try { fs.writeFileSync(STATS_PATH, JSON.stringify(s, null, 2)); } catch {} }
let _convsCache = null;
let _convsSaveTimer = null;

function loadConversations(convPath) {
  const CONV_PATH = convPath || STANDALONE_DATA + '/conversations.json';
  if (_convsCache) return _convsCache;
  try { _convsCache = JSON.parse(fs.readFileSync(CONV_PATH, 'utf8')); }
  catch { _convsCache = {}; }
  return _convsCache;
}
function saveConversations(c, convPath) {
  const CONV_PATH = convPath || STANDALONE_DATA + '/conversations.json';
  _convsCache = c;
...
function saveConversationsNow(c, convPath) {
  const CONV_PATH = convPath || STANDALONE_DATA + '/conversations.json';
...
class WhatsAppBot {
  constructor(io, dataDir, userId) {
    ...
    this.remarketingInProgress = new Set(); // evita envio duplicado de remarketing
    this.remarketingTimer = null; // referência do timer para evitar duplicatas
    global._botInstance = this; // usado pelos handlers de sinal para salvar sessões
    this.debounceTimers = {};
    ...
    this.archivedJids = new Set(); // cache em memória para lookup instantâneo
  }
  ...
  startSessionSaving() {
    // Cancela timer anterior antes de criar novo (evita duplicatas ao reconectar)
    ...
    }, 30 * 1000); // salva a cada 30s
    console.log('[BOT] Auto-save de sessões ativado (30 em 30s)');
  }

  initAI() {
    const _saved = (() => { try { return JSON.parse(fs.readFileSync(this.FLOWS_PATH,'utf8')); } catch { return {}; } })();
    const key = _saved.anthropicApiKey || process.env.ANTHROPIC_API_KEY;
    if (key && key !== 'sk-ant-SUA_CHAVE_AQUI') {
      this.ai = new Anthropic({ apiKey: key });
    }
    const groqKey = _saved.groqApiKey || process.env.GROQ_API_KEY;
  ...
  setStatus(status, extra = {}) {
    this.status = status;
    const _t = (this.userId && this.io && this.io.to) ? this.io.to('user:' + this.userId) : this.io;
    if (_t) _t.emit('bot:status', { status, ...extra });
  ...
  async checkRemarketingLeads() {
    ...
    try {
      const cfg = loadFlows(this.FLOWS_PATH);
      ...
      const convs = loadConversations(this.CONV_PATH);
  ...
  async sendTextHuman(jid, text, incomingText) {
    ...
    try {
      ...
      await this.sock.sendPresenceUpdate('paused', jid);
      const textFinal = this.injectPaymentRef ? this.injectPaymentRef(text, jid) : text;
      await this.sock.sendMessage(jid, { text: textFinal });
      await sleep(350);
    } catch {}
  }
  ...
  recordMessage(jid, role, text, flowName) {
    try {
      ...
      const stats = loadStats(this.STATS_PATH);
      ...
      saveStats(stats, this.STATS_PATH);
  ...
  async processMessage(jid, text) {
    console.log('[BOT] processMessage jid=' + jid.slice(-8) + ' text=' + text.slice(0,30));
      ...
      if (!session.started) {
        ...
        if (startFlow) {
          ...
          if (this.ai) {
            ...
            const cfg2 = loadFlows(this.FLOWS_PATH);
  ...
  async start() {
    if (this.status === 'connected') return;
    // Fecha socket anterior antes de criar novo (evita event handlers duplicados)
    if (this.sock) {
      try { this.sock.end(undefined); } catch {}
      this.sock = null;
      await new Promise(r => setTimeout(r, 500)); // aguarda limpeza
    }
    this.initAI();
    this.setStatus('connecting');
    // Só carrega sessões e inicia timers na PRIMEIRA vez
    ...
    this.sock.ev.on('connection.update', async (update) => {
      ...
      if (connection === 'close') {
        ...
        // Salva tudo imediatamente ao desconectar — evita perda de dados
        try {
          this.saveSessions();
          if (_convsCache) saveConversationsNow(_convsCache);
          console.log('[BOT] Dados salvos após desconexão.');
        } catch(e) { console.error('[BOT] Erro ao salvar na desconexão:', e.message); }
        if (code === DisconnectReason.loggedOut) {
          console.log('[BOT] ✗ Sessão encerrada — reconecte pelo dashboard');
          setTimeout(() => process.exit(1), 1000);
        } else if (this._wasConnected) {
          // Só sai se já tinha conectado antes — evita sair durante exibição do QR
          console.log('[BOT] ⚠ Saindo para reinício limpo pelo PM2...');
          setTimeout(() => process.exit(1), 1000);
        } else {
          // Ainda não conectou (aguardando QR) — tenta de novo
      ...
      if (connection === 'open') {
        this.reconnectAttempts = 0;
        this._wasConnected = true; // marca que já conectou ao menos uma vez
        this.qrDataUrl = null;
        this.setStatus('connected');
        this.startRemarketingTimer(); // reinicia remarketing a cada conexão (com delay de segurança)
      }
    });
    ...
    this.sock.ev.on('chats.update', (updates) => {
      let convs = loadConversations(this.CONV_PATH);
      let changed = false;
    ...
    this.sock.ev.on('messages.upsert', async ({ messages, type }) => {
      // Debug: confirma que handler está ativo após reconexão
      if (type === 'notify') console.log('[BOT] messages.upsert notify: ' + messages.length + ' msg(s)');
      ...
      // type: notify = mensagem nova (online ou offline acumulada)
      // type: append = histórico de sync — ignorar para evitar flood
      ...
      for (const msg of messages) {
        ...
        // Ignora mensagens com mais de 10 minutos (evita flood de retries antigos)
        // 10 minutos = mensagens que chegaram enquanto o bot estava offline ainda são processadas
        if (msgTime > 0 && (now - msgTime) > 600) continue;
  ...
  // ── SaaS attribution methods ──────────────────────────────────────────────────
  captureTrackingFromMessage(jid, text) {
    const session = this.userSessions[jid];
    if (!session || session.firstMessage) return;
    session.firstMessage = (text || '').substring(0, 300);
    const urlMatch = text && text.match(/https?:\/\/[^\s]+/);
    if (urlMatch) {
      try {
        const url = new URL(urlMatch[0]);
        ['utm_source','utm_medium','utm_campaign','utm_content','utm_term','fbclid'].forEach(k => {
          if (url.searchParams.get(k) && !session[k]) session[k] = url.searchParams.get(k);
        });
      } catch {}
    }
  }

  injectPaymentRef(text, jid) {
    if (!text || !jid) return text;
    return text.replace(/(https?:\/\/[^\s]*\/pay\/[^\s?#]*)/g, match => {
      try {
        const sep = match.includes('?') ? '&' : '?';
        return match + sep + 'wjid=' + encodeURIComponent(jid.replace('@s.whatsapp.net',''));
      } catch { return match; }
    });
  }

  getSessionTracking(jid) {
    const s = this.userSessions[jid];
    if (!s) return null;
    return { utm_source:s.utm_source||'', utm_medium:s.utm_medium||'', utm_campaign:s.utm_campaign||'', fbclid:s.fbclid||'', phone:jid.replace('@s.whatsapp.net','') };
  }
...
function _saveAndExit(signal) {
  ...
  try {
    if (global._botInstance) {
      ...
      if (stats) saveStats(stats, this.STATS_PATH);
    }
...
process.on('exit', () => {
  try {
    if (global._botInstance) {
      global._botInstance.saveSessions();
      const c = global._botInstance._pendingConvs; if (c) saveConversations(c, this.CONV_PATH);
      const s = global._botInstance._pendingStats; if (s) saveStats(s, this.STATS_PATH);
    }
  } catch {}
});
