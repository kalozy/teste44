...
router.get('/orders/summary', (req, res) => {
  ...
  const row = db.prepare(`
    ...
    FROM orders WHERE created_at BETWEEN ? AND ? AND (user_id = ? OR user_id IS NULL)
  `).get(dateFrom, dateTo, req.user?.id || 0);
  res.json(row);
});
...
router.get('/orders/by-campaign', (req, res) => {
  ...
  let sql = `
    ...
    FROM orders WHERE utm_campaign IS NOT NULL AND utm_campaign != '' AND (user_id = ? OR user_id IS NULL)
  `;
  const params = [];
  ...
  res.json(db.prepare(sql).all(req.user?.id || 0, ...params));
});
...
router.get('/meta-config', (req, res) => {
  ...
  const cfg = db.prepare('SELECT * FROM meta_config WHERE user_id = ?').get(uid)
           || db.prepare('SELECT * FROM meta_config WHERE id = 1').get(); // legacy fallback
...
router.post('/meta-config', (req, res) => {
  ...
  const existing = db.prepare('SELECT id FROM meta_config WHERE user_id = ?').get(uid);
  if (existing) {
    db.prepare(`UPDATE meta_config SET access_token=?, ad_account_id=?, pixel_id=? WHERE user_id=?`).run(access_token||'', ad_account_id||'', pixel_id||'', uid);
  } else {
    db.prepare('INSERT INTO meta_config (user_id, access_token, ad_account_id, pixel_id) VALUES (?,?,?,?)').run(uid, access_token||'', ad_account_id||'', pixel_id||'');
  }
  res.json({ ok: true });
});
...
router.get('/meta/campaigns', async (req, res) => {
  const uid = req.user?.id || 0;
  const cfg = db.prepare('SELECT * FROM meta_config WHERE user_id = ?').get(uid)
           || db.prepare('SELECT * FROM meta_config WHERE id = 1').get();
  if (!cfg || !cfg.access_token || !cfg.ad_account_id) return res.json({ data: [], error: 'Meta Ads não configurado' });
  const { from, to } = req.query;
